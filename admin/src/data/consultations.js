...
const GOAL_DATA_BY_GOAL = {
    ...
    'mother-wellness': (i) => ({
        postpartumStage: ['0-6-months', '6-12-months', '1-2-years', '2-plus-years'][i % 4],
        numChildren: (i % 3) + 1,
        deliveryType: ['vaginal', 'c-section'][i % 2],
        breastfeeding: ['exclusively', 'partially', 'not', 'na'][i % 4],
        conditions: ['Anemia', 'Thyroid Issues', 'None'].slice(0, (i % 3) + 1),
        medications: 'Multivitamins, iron supplement',
        meals: 3 + (i % 2),
        water: 2 + (i % 3) * 0.5,
        sleep: 5 + (i % 4),
        energy: 4 + (i % 5),
        goals: ['Postpartum Weight Loss', 'Regain Energy', 'Balanced Nutrition'].slice(0, (i % 3) + 1),
        notes: 'Wants a routine that fits around caring for a newborn at home.',
    }),
    diabetes: (i) => ({
